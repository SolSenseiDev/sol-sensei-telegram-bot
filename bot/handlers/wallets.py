...
from bot.services.rust_swap import withdraw_sol_txid
from bot.services.rust_swap import withdraw_usdc_txid
...
@wallets_router.callback_query(F.data == "withdraw_sol")
...
@wallets_router.message(WalletStates.waiting_for_withdraw_amount)
...
@wallets_router.callback_query(F.data == "withdraw_usdc")
...
@wallets_router.message(WalletStates.waiting_for_withdraw_usdc_address)
...
@wallets_router.message(WalletStates.waiting_for_withdraw_usdc_amount)
async def handle_withdraw_usdc_amount(message: Message, state: FSMContext):
    ...
    for wallet in wallets:
        try:
            ...
            failed[wallet.address] = "Ошибка проверки баланса"
            continue
