...
#[allow(dead_code)]
pub async fn sell_token_for_sol(base58_str: &str, token_mint: &str, amount: u64) -> Result<()> {
    let keypair = decode_keypair(base58_str)?;
...
async fn swap_directional(
    ...
    // Создание WSOL ATA при необходимости
    ...
    respond_empty(Err(anyhow!("All swap routes failed")));
    Ok(())
}
